...
void missing_filename_error(const char *programname)
{
	...
	fprintf(stderr, BLUE "If you unsure what assembly code to pass use our own defined\n" RESET);
	fprintf(
	    stderr, "\t" GREEN "%s" RESET " " MAGENTA "-e" RESET " " YELLOW "%s\n\n" RESET, programname,
	    "ifelse.s");
	fprintf(stderr, DIM "you can use other assembly located in example directory.\n" RESET);
...
void unable_to_allocate_memory_error(const char *variable_name)
{
	fprintf(
	    stderr,
	    RED BOLD "Memory Error: " RESET RED "Unable to allocate memory [" YELLOW "%s" RED
	             "]\n" RESET,
	    variable_name);
	shutdown(-1);
}
...
{
	fprintf(stderr, RED BOLD "Syntax Error: " RESET RED "unrecognized assembly syntax.\n" RESET);
...
void segmentation_fault_error(const char *error_msg)
{
	fprintf(
	    stderr,
	    RED BOLD "Segmentation Fault: " RESET RED "occurred when executing your assembly: " YELLOW
	             "%s\n" RESET,
	    error_msg);
	shutdown(-1);
}
...
{
	fprintf(stderr, RED BOLD "File Error: " RESET RED "Could not open: " YELLOW "%s\n" RESET, file);
	shutdown(-1);
}
...
void unknown_opcode_error(char **instruction, const char *alt)
{
	...
	if (alt) { fprintf(stderr, GREEN "Did you mean: " CYAN "%s\n" RESET, alt); }
	shutdown(-1);
}
