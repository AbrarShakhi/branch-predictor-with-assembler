...
bool valid_jump(int dest, int ilen) { return dest >= 0 && dest <= ilen; }
...
void print_cpu_debug_state(Cpu *cpu, Predictor *predictor, bool predicted_taken,
  ...
  fprintf(stdout,
          CYAN "PC: " RESET GREEN "%d" RESET "  " CYAN "MAR: " RESET GREEN
               "%d" RESET "  " CYAN "IR: " RESET YELLOW "\"",
          cpu->pc, cpu->mar);

  for (int i = 0; i < arrlen(cpu->ir); i++) {
    fprintf(stdout, "%s", cpu->ir[i]);
    if (i < arrlen(cpu->ir) - 1) {
      fprintf(stdout, " ");
    }
  }
  ...
  fprintf(stdout,
          CYAN "ALU EG: " RESET GREEN "%d" RESET "  " CYAN
               "ALU NEG: " RESET GREEN "%d" RESET "  " CYAN "AC: " RESET YELLOW
               "%d" RESET "\n",
          cpu->alu.eq, cpu->alu.neg, cpu->ac);

  fprintf(stdout, BLUE "Predicted: " RESET "%s" RESET "  ",
          predicted_taken ? GREEN "TAKEN" : RED "NOT TAKEN");

  fprintf(stdout, BLUE "Actual: " RESET "%s" RESET,
          actual_taken ? GREEN "TAKEN" : RED "NOT TAKEN");
  ...
  if (entry && entry->value) {
    fprintf(stdout,
            YELLOW "Predictor Counter [instr %d]: " RESET CYAN "%d\n" RESET,
            cpu->mar, entry->value->counter);
  } else {
    fprintf(stdout,
            YELLOW "Predictor Counter [instr %d]: " RESET RED "N/A\n" RESET,
            cpu->mar);
  }
...
void cpu_interpret(Cpu *cpu) {
  while (!__cpu_process_finish(cpu)) {
    ...
    int next_instr = __cpu_execute_instruction(
        cpu,
        __cpu_decode_instruction(cpu, __cpu_fetch_instruction(cpu, cpu->mar),
                                 &is_jump),
